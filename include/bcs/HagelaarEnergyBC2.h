...
#ifndef HAGELAARENERGYBC2_H
#define HAGELAARENERGYBC2_H

#include "IntegratedBC.h"

class HagelaarEnergyBC2;

template <>
InputParameters validParams<HagelaarEnergyBC2>();

class HagelaarEnergyBC2 : public IntegratedBC
{
public:
  HagelaarEnergyBC2(const InputParameters & parameters);
...
protected:
  ...
  unsigned int _ip_id;
  MooseVariable & _ip2_var;
  const VariableValue & _ip2;
  const VariableGradient & _grad_ip2;
  unsigned int _ip2_id;
  ...
  const MaterialProperty<Real> & _sgnip2;
  const MaterialProperty<Real> & _muip2;
  const MaterialProperty<Real> & _Dip2;
