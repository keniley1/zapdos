...
registerMooseObject("ZapdosApp", ADHenryInterface);

InputParameters
ADHenryInterface::validParams()
{
...
ADHenryInterface::ADHenryInterface(const InputParameters & parameters)
  : ADInterfaceKernel(parameters),
    _r_units(1. / getParam<Real>("position_units")),
    _r_neighbor_units(1. / getParam<Real>("neighbor_position_units")),
    _henry_coeff(getParam<Real>("h")),

    _diff(getNeighborADMaterialProperty<Real>("diff" + _neighbor_var.name()))
{
  if (!parameters.isParamValid("boundary"))
  {
    mooseError("In order to use the ADHenryInterface dgkernel, you must specify a "
               "boundary where it will live.");
  }
}

ADReal
ADHenryInterface::computeQpResidual(Moose::DGResidualType type)
{
