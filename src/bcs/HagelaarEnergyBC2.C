...
#include "HagelaarEnergyBC2.h"

// MOOSE includes
#include "MooseVariable.h"

registerMooseObject("ZapdosApp", HagelaarEnergyBC2);

template <>
InputParameters
validParams<HagelaarEnergyBC2>()
{
  ...
  params.addRequiredCoupledVar("ip2", "The second ion density.");
  params.addRequiredParam<Real>("position_units", "Units of position.");
  return params;
}

HagelaarEnergyBC2::HagelaarEnergyBC2(const InputParameters & parameters)
  : IntegratedBC(parameters),
    ...
    _ip_id(coupled("ip")),
    _ip2_var(*getVar("ip2", 0)),
    _ip2(coupledValue("ip2")),
    _grad_ip2(coupledGradient("ip2")),
    _ip2_id(coupled("ip2")),
    ...
    _sgnip2(getMaterialProperty<Real>("sgn" + _ip2_var.name())),
    _muip2(getMaterialProperty<Real>("mu" + _ip2_var.name())),
    _Dip2(getMaterialProperty<Real>("diff" + _ip2_var.name())),
...
Real
HagelaarEnergyBC2::computeQpResidual()
{
  ...
  _ion_flux = (_sgnip[_qp] * _muip[_qp] * -_grad_potential[_qp] * _r_units * std::exp(_ip[_qp]) -
              ...
              (_sgnip2[_qp] * _muip2[_qp] * -_grad_potential[_qp] * _r_units * std::exp(_ip2[_qp]) -
               _Dip2[_qp] * std::exp(_ip2[_qp]) * _grad_ip2[_qp] * _r_units);
...
Real
HagelaarEnergyBC2::computeQpJacobian()
{
  if (_normals[_qp] * -1.0 * -_grad_potential[_qp] > 0.0)
  {
    _a = 1.0;
...
Real
HagelaarEnergyBC2::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _potential_id)
  {
    ...
    _ion_flux =
        (_sgnip[_qp] * _muip[_qp] * -_grad_potential[_qp] * _r_units * std::exp(_ip[_qp]) -
         _Dip[_qp] * std::exp(_ip[_qp]) * _grad_ip[_qp] * _r_units) +
        (_sgnip2[_qp] * _muip2[_qp] * -_grad_potential[_qp] * _r_units * std::exp(_ip2[_qp]) -
         _Dip2[_qp] * std::exp(_ip2[_qp]) * _grad_ip2[_qp] * _r_units);
    _d_ion_flux_d_potential = (_sgnip[_qp] * _muip[_qp] * std::exp(_ip[_qp]) +
                               _sgnip2[_qp] * _muip2[_qp] * std::exp(_ip2[_qp])) *
  ...
  else if (jvar == _ip2_id)
  {
    ...
    _v_thermal =
        std::sqrt(8 * _e[_qp] * 2.0 / 3 * std::exp(_u[_qp] - _em[_qp]) / (M_PI * _massem[_qp]));
    _d_ion_flux_d_ip = _sgnip2[_qp] * _muip2[_qp] * -_grad_potential[_qp] * _r_units *
                           std::exp(_ip2[_qp]) * _phi[_j][_qp] -
                       _Dip2[_qp] * std::exp(_ip2[_qp]) * _grad_phi[_j][_qp] * _r_units -
                       _Dip2[_qp] * std::exp(_ip2[_qp]) * _phi[_j][_qp] * _grad_ip2[_qp] * _r_units;
    // _d_n_gamma_d_ip = (1. - _a) * _se_coeff[_qp] * _d_ion_flux_d_ip * _normals[_qp] / (_muem[_qp]
    // * -_grad_potential[_qp] * _r_units * _normals[_qp] + std::numeric_limits<double>::epsilon());
