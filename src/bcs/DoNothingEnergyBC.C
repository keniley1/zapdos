...
#include "DoNothingEnergyBC.h"

// MOOSE includes
#include "MooseVariable.h"

registerMooseObject("ZapdosApp", DoNothingEnergyBC);

template <>
InputParameters
validParams<DoNothingEnergyBC>()
{
...
DoNothingEnergyBC::DoNothingEnergyBC(const InputParameters & parameters)
...
DoNothingEnergyBC::~DoNothingEnergyBC() {}
...
Real
DoNothingEnergyBC::computeQpJacobian()
{
  ...
  return ((_d_muel_d_u * _sign[_qp] * std::exp(_u[_qp]) * -_grad_potential[_qp] * _r_units *
            ...
            _muel[_qp] * _sign[_qp] * std::exp(_u[_qp]) * _phi[_j][_qp] * -_grad_potential[_qp] *
               ...
               _test[_i][_qp] * _r_units -
           _diffel[_qp] *
               (std::exp(_u[_qp]) * _grad_phi[_j][_qp] * _r_units * -_normals[_qp] +
                std::exp(_u[_qp]) * _phi[_j][_qp] * _grad_u[_qp] * _r_units * -_normals[_qp]) *
               _test[_i][_qp] * _r_units -
           _d_diffel_d_u * std::exp(_u[_qp]) * _grad_u[_qp] * _r_units * -_normals[_qp] *
               _test[_i][_qp] * _r_units);
}

Real
DoNothingEnergyBC::computeQpOffDiagJacobian(unsigned int jvar)
{
