...
#include "ADSecondaryElectronEnergyComsolBC.h"

// MOOSE includes
#include "MooseVariable.h"

registerADMooseObject("ZapdosApp", ADSecondaryElectronEnergyComsolBC);

defineADLegacyParams(ADSecondaryElectronEnergyComsolBC);

InputParameters
ADSecondaryElectronEnergyComsolBC::validParams()
{
...
ADSecondaryElectronEnergyComsolBC::ADSecondaryElectronEnergyComsolBC(
...
ADReal
ADSecondaryElectronEnergyComsolBC::computeQpResidual()
{
  _ion_flux.zero();
  for (unsigned int i = 0; i < _num_ions; ++i)
  {
  ...
  return -_test[_i][_qp] * _se_coeff * _ion_flux * _normals[_qp] * _se_energy[_qp];
}
