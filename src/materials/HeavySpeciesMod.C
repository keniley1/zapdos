...

{
  ...
  {
    mooseError("User must input AT LEAST ONE of the parameters 'mobility' and 'diffusivity' to "
               "HeavySpeciesMod!");
  }
}
...
HeavySpeciesMod::computeQpProperties()
{
  ...
  else if (!_calc_mobility && _calc_diffusivity)
  {
    _muHeavy[_qp] = getParam<Real>("mobility") * _voltage_scaling * _time_units;
    _diffHeavy[_qp] =
        _muHeavy[_qp] * _temperatureHeavy[_qp] * 1.3807e-23 / (_user_sgnHeavy * 1.602e-19);
  }
  else
  {
  ...
  {
    _diffHeavy[_qp] = 0.004 * _time_units / (760. * _p_gas[_qp] / 1.01E5); // covert to m^2 and
  include press
  }
  */
}
