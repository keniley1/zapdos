...
#include "ADHeavySpeciesMaterial.h"
#include "MooseUtils.h"

registerADMooseObject("ZapdosApp", ADHeavySpeciesMaterial);

defineADLegacyParams(ADHeavySpeciesMaterial);

template <ComputeStage compute_stage>
InputParameters
ADHeavySpeciesMaterial<compute_stage>::validParams()
{
...
template <ComputeStage compute_stage>
ADHeavySpeciesMaterial<compute_stage>::ADHeavySpeciesMaterial(const InputParameters & parameters)
  : ADMaterial<compute_stage>(parameters),
    _user_massHeavy(getParam<Real>("heavy_species_mass")),
    _user_sgnHeavy(getParam<Real>("heavy_species_charge")),
    _potential_units(getParam<std::string>("potential_units")),
    _massHeavy(declareADProperty<Real>("mass" + getParam<std::string>("heavy_species_name"))),
    _temperatureHeavy(declareADProperty<Real>("T" + getParam<std::string>("heavy_species_name"))),
    _sgnHeavy(declareADProperty<Real>("sgn" + getParam<std::string>("heavy_species_name"))),
    _muHeavy(declareADProperty<Real>("mu" + getParam<std::string>("heavy_species_name"))),
    _diffHeavy(declareADProperty<Real>("diff" + getParam<std::string>("heavy_species_name"))),
    _T_gas(getMaterialProperty<Real>("T_gas")),
    _p_gas(getMaterialProperty<Real>("p_gas")),
    _time_units(getParam<Real>("time_units")),
    _grad_mu(declareADProperty<RealVectorValue>("grad_mu" +
                                                getParam<std::string>("heavy_species_name"))),
    _grad_diff(declareADProperty<RealVectorValue>("grad_diff" +
                                                  getParam<std::string>("heavy_species_name")))
...
template <ComputeStage compute_stage>
void
ADHeavySpeciesMaterial<compute_stage>::computeQpProperties()
{
