...
registerADMooseObject("ZapdosApp", SurfaceCharge);

InputParameters
SurfaceCharge::validParams()
{
    ...
    _grad_em(adCoupledGradient("em"))
{
  ...
  if (_include_secondary_electrons && !isParamValid("se_coeff"))
    mooseError("SurfaceCharge2 material: secondary_electrons is set to true, but no secondary "
  ...
  else if (bc == "Lymberopoulos")
  {
    _bc_type = 2;
    if (!isParamValid("ks"))
      mooseError("SurfaceCharge2: bc_type = Lymberopoulos requires the electron recombination "
  ...
  else if (bc == "Test")
  {
    ...
    mooseError(
        "SurfaceCharge: bc_type = " + getParam<std::string>("bc_type") +
  ...
  for (unsigned int i = 0; i < _ions.size(); ++i)
  {
    ...
    _diff_ions[i] = &getADMaterialProperty<Real>("diff" + (*getVar("ions", i)).name());
...
SurfaceCharge::computeQpProperties()
{
  if (_material_data_type == Moose::FACE_MATERIAL_DATA || boundaryRestricted())
  {
    ...
    // The total plasma current to the wall is included as an optional output.
    _plasma_current[_qp] =
...
void
SurfaceCharge::computeTest()
{
  _electron_flux = std::exp(_em[_qp]) *
                   (-_muem[_qp] * -_grad_potential[_qp] * _r_units * _normals[_qp] - _diffem[_qp] * _grad_em[_qp] * _r_units * _normals[_qp]);

  for (unsigned int i = 0; i < _num_ions; ++i)
  {
    _ion_charge_flux += 
        std::exp((*_ions[i])[_qp]) * ((*_sgn_ions[i])[_qp] * (*_mu_ions[i])[_qp] * -_grad_potential[_qp] * _r_units * _normals[_qp] - (*_diff_ions[i])[_qp] * (*_grad_ions[i])[_qp] * _r_units * _normals[_qp]) * (*_sgn_ions[i])[_qp]; 
  }
